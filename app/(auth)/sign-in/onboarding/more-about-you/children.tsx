...
import { childrenOptions, Children } from "@/types";
...
export default function smokingHabits() {
    ...
    const mutation = useMutation({
        mutationFn: async () => updateUser(user?.id ?? "", children as Children),
        onError: (error) => {
    ...
    useEffect(() => {
        if (pathName === "/sign-in/onboarding/more-about-you/children") {
            setFabState({
                isDisabled: false,
                onPress: () => {
                    router.push("/sign-in/onboarding/more-about-you/pets");
                    if (children && children !== user?.children) {
                        mutation.mutate()
                    }
                }
            })
...
async function updateUser(userId: string, children: Children) {
    const {error} = await supabase.from('user_additional_info').update({children: children}).eq('id', userId);
