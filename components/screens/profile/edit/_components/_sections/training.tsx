...
import { DrinkingHabits, drinkingHabitsOptions, trainingHabitsOptions } from "@/types";
    ...
    <>
        ...
        >
            <VStack className="gap-4 p-4 mb-1 bg-background-50 rounded-lg">
                <Text className="text-typography-600 text-sm">{i18n.t("onboarding.moreAboutYou.training.title")}</Text>
                <Text className="text-typography-950">{i18n.t(`onboarding.moreAboutYou.training.options.${user.training_habits}`)}</Text>
            </VStack>
        </Pressable>
...
function EditSheet({isOpen, setIsOpen}: {isOpen: boolean, setIsOpen: (value: boolean) => void}) {
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.setQueryData(['user', 'full'], {...user, training_habits: value})
    ...
    return (
        ...
        onClose={() => {
            ...
            const isDirty = user.training_habits !== value;
            if (isDirty) {
                mutation.mutate();
            }
        }}
...
async function updateUser(userId: string, value: string) {
    const {error} = await supabase.from('user_additional_info').update({training_habits: value}).eq('id', userId);
