...
import { childrenOptions, DrinkingHabits, drinkingHabitsOptions, foodChoicesOptions, trainingHabitsOptions } from "@/types";
    ...
    <>
        ...
        >
            <VStack className="gap-4 p-4 mb-1 bg-background-50 rounded-lg">
                <Text className="text-typography-600 text-sm">{i18n.t("onboarding.moreAboutYou.children.title")}</Text>
                <Text className="text-typography-950">{i18n.t(`onboarding.moreAboutYou.children.options.${user.children}`)}</Text>
            </VStack>
        </Pressable>
...
function EditSheet({isOpen, setIsOpen}: {isOpen: boolean, setIsOpen: (value: boolean) => void}) {
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.setQueryData(['user', 'full'], {...user, children: value})
    ...
    return (
        ...
        onClose={() => {
            ...
            const isDirty = user.children !== value;
            if (isDirty) {
                mutation.mutate();
            }
        }}
       ...
       >
            ...
            <BottomSheetContent className="border-primary-0 bg-background-0 px-5 flex-1 h-full" >
                ...
                <FormControl className="w-full gap-6">
                    ...
                    >
                        {i18n.t("onboarding.moreAboutYou.children.title")}
                    </AnimatedHeader>
                    <BottomSheetScrollView 
                        ...
                        >
                            {childrenOptions.map((option, index) => 
                                ...
                                >
                                    ...
                                    >
                                        {i18n.t(`onboarding.moreAboutYou.children.options.${option}`)}
                                    </AnimatedRadioLabel>
...
async function updateUser(userId: string, value: string) {
    const {error} = await supabase.from('user_additional_info').update({children: value}).eq('id', userId);
