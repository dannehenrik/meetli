...
import { DrinkingHabits, drinkingHabitsOptions, foodChoicesOptions, politicalViewOptions, trainingHabitsOptions } from "@/types";
    ...
    <>
        ...
        >
            <VStack className="gap-4 p-4 mb-1 bg-background-50 rounded-lg">
                <Text className="text-typography-600 text-sm">{i18n.t("onboarding.moreAboutYou.politicalView.title")}</Text>
                <Text className="text-typography-950">{i18n.t(`onboarding.moreAboutYou.politicalView.options.${user.political_view}`)}</Text>
            </VStack>
        </Pressable>
...
function EditSheet({isOpen, setIsOpen}: {isOpen: boolean, setIsOpen: (value: boolean) => void}) {
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.setQueryData(['user', 'full'], {...user, political_view: value})
    ...
    return (
        ...
        onClose={() => {
            ...
            const isDirty = user.political_view !== value;
            if (isDirty) {
                mutation.mutate();
            }
        }}
...
async function updateUser(userId: string, value: string) {
    const {error} = await supabase.from('user_additional_info').update({political_view: value}).eq('id', userId);
