    ...
    <>
        ...
        >
            <VStack className="gap-4 p-4 mb-1 bg-background-50 rounded-lg">
                <Text className="text-typography-600 text-sm">{i18n.t("onboarding.moreAboutYou.religion.title")}</Text>
                <Text className="text-typography-950">{i18n.t(`onboarding.moreAboutYou.religion.options.${user.religion}`)}</Text>
            </VStack>
        </Pressable>
...
function EditSheet({isOpen, setIsOpen}: {isOpen: boolean, setIsOpen: (value: boolean) => void}) {
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.setQueryData(['user', 'full'], {...user, religion: value})
    ...
    return (
        ...
        onClose={() => {
            ...
            const isDirty = user.religion !== value;
            if (isDirty) {
                mutation.mutate();
            }
        }}
...
async function updateUser(userId: string, value: string) {
    const {error} = await supabase.from('user_additional_info').update({religion: value}).eq('id', userId);
