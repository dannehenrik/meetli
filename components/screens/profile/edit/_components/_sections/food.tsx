...
import { DrinkingHabits, drinkingHabitsOptions, foodChoicesOptions, trainingHabitsOptions } from "@/types";
    ...
    <>
        ...
        >
            <VStack className="gap-4 p-4 mb-1 bg-background-50 rounded-lg">
                <Text className="text-typography-600 text-sm">{i18n.t("onboarding.moreAboutYou.foodChoice.title")}</Text>
                <Text className="text-typography-950">{i18n.t(`onboarding.moreAboutYou.foodChoice.options.${user.food_choice}`)}</Text>
            </VStack>
        </Pressable>
...
function EditSheet({isOpen, setIsOpen}: {isOpen: boolean, setIsOpen: (value: boolean) => void}) {
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.setQueryData(['user', 'full'], {...user, food_choice: value})
    ...
    return (
        ...
        onClose={() => {
            ...
            const isDirty = user.food_choice !== value;
            if (isDirty) {
                mutation.mutate();
            }
        }}
...
async function updateUser(userId: string, value: string) {
    const {error} = await supabase.from('user_additional_info').update({food_choice: value}).eq('id', userId);
